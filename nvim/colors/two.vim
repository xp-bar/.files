" Name:    two vim colorscheme
" Author:  Ramzi Akremi, edited by Nicholas Ireland
" License: MIT
" Version: 1.1.1-pre
...
let g:colors_name = 'two'

if !exists('g:two_allow_italics')
  let g:two_allow_italics = 0
endif
...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  fun <sid>X(group, fg, bg, attr)
    let l:attr = a:attr
    if g:two_allow_italics == 0 && l:attr ==? 'italic'
      let l:attr= 'none'
    endif
  ...
  if &background ==# 'dark'
    ...
    let s:hue_1  = '56b6c2' " cyan #56b6c2
    let s:hue_2  = '61afef' " blue #61afef
    let s:hue_3  = 'c678dd' " purple #c678dd
    let s:hue_4  = '98c379' " green #98c379

    let s:hue_5   = 'e06c75' " red 1 #e06c75
    let s:hue_5_2 = 'be5046' " red 2 #be5046

    let s:hue_6   = 'd19a66' " orange 1 #d19a66
  ...
  call <sid>X('Operator',       s:hue_5,         '',          'none')
  ...
  " BufTabLine highlighting ----------------------------------------------------{{{
  call <sid>X('BufTabLineCurrent',     s:hue_3, '', '') " Buffer shown in current window
  call <sid>X('BufTabLineActive',      s:syntax_gutter, '', '') " Buffer shown in other window
  call <sid>X('BufTabLineHidden',      s:syntax_gutter, '', '') " Buffer not currently visible
  call <sid>X('BufTabLineFill',        '', 'none', '') " Empty area
  "}}}
  ...
  call <sid>X('phpClasses',                 s:hue_6_2,   '', '')
  call <sid>X('phpStaticClasses',           s:hue_6_2,   '', '')
  call <sid>X('phpClassExtends',            s:hue_4,     '', '')
  call <sid>X('phpClassImplements',         s:hue_4,     '', '')
  call <sid>X('phpUseAlias',                s:hue_6_2,     '', '')
  call <sid>X('phpFunction',                s:hue_2,     '', '')
  call <sid>X('phpFunctions',               s:hue_1,     '', '')
  call <sid>X('phpStatement',               s:hue_1,     '', '')
  call <sid>X('phpMethodsVar',              s:hue_5,     '', '')
  call <sid>X('phpMemberSelector',          s:syntax_fg, '', '')
  call <sid>X('phpOperator',                s:syntax_fg, '', '')
  call <sid>X('phpMemberVariable',          s:hue_3, '', '')
  call <sid>X('phpSplatOperator',           s:syntax_fg, '', '')
  call <sid>X('phpVarSelector',             s:hue_5,     '', '')
  call <sid>X('phpInclude',                 s:hue_3,     '', '')
  call <sid>X('phpConstants',               s:hue_6,     '', '')
  call <sid>X('phpKeyword',                 s:hue_3,     '', '')
  call <sid>X('phpBoolean',                 s:hue_6,     '', '')
  call <sid>X('phpParent',                  s:mono_3,    '', '')
  call <sid>X('phpType',                    s:hue_3,     '', '')
  call <sid>X('phpDocTags',                 s:hue_3,     '', '')
  call <sid>X('phpDocCustomTags',           s:hue_3,     '', '')
  call <sid>X('phpException',               s:hue_6_2,   '', '')
  call <sid>X('phpSuperGlobals',            s:hue_5,     '', '')
  call <sid>X('phpParent',                  s:syntax_fg,     '', '')
  call <sid>X('DbgCurrentLine',             '', '', '')
  call <sid>X('DbgCurrentSign',             s:hue_4, '', '')
  call <sid>X('DbgBreakptLine',             '', '', '')
...
"}}}
" Public API --------------------------------------------------------------{{{
function! two#highlight(group, fg, bg, attr)
